...
SITE_PROMPT = (
    "Jesteś ekspertem od analizy konkurencji dla firm B2B. "
    "Analizujesz strony internetowe konkurentów i dajesz właścicielom firm konkretne, praktyczne wnioski. "
    "Oceniasz: stronę www, SEO, ofertę, widoczność online. "
    "Piszesz po polsku, konkretnie i profesjonalnie. "
    "Zwróć WYŁĄCZNIE surowy obiekt JSON (bez markdown, bez backticks).\n\n"
    "Struktura JSON:\n"
    "{\n"
    '  "brand_name": "nazwa firmy",\n'
    '  "website_score": 0,\n'
    '  "seo_score": 0,\n'
    '  "offers_score": 0,\n'
    '  "keywords": {\n'
    ...
    '  },\n'
    '  "findings": {\n'
    '    "good": ["2-3 mocne strony tej witryny — konkretnie i profesjonalnie"],\n'
    '    "warn": ["2-3 rzeczy do poprawy"],\n'
    '    "bad": ["1-2 wyraźne słabości"]\n'
    '  }\n'
    "}\n\n"
    "Zasady: score = liczby całkowite 0-100. Cały tekst po polsku, konkretnie i profesjonalnie."
)

COMPARE_PROMPT = (
    "Jesteś ekspertem od analizy konkurencji dla firm B2B. "
    "Porównaj wyniki analizy strony KONKURENCJI i MOJEJ firmy. "
    ...
    '    "np. Twoja strona lepiej opisuje oferowane usługi"\n'
    '  ],\n'
    '  "actions": [\n'
    ...
    "Zamiast 'dodaj treści' pisz 'Stwórz sekcję z opiniami klientów — minimum 5 opinii z nazwą firmy'. "
    "Wszystko po polsku, językiem właściciela firmy B2B."
)
...
def _call_claude(system, user_msg, max_tokens=4096):
...
def analyze_site(website_text):
    text = _call_claude(SITE_PROMPT, f"Przeanalizuj tę stronę:\n\n{website_text}")
    return _validate_site(_parse_json(text))


def compare_sites(competitor, mine):
    user_msg = (
        f"Dane strony KONKURENCJI:\n{json.dumps(competitor, ensure_ascii=False)}\n\n"
        f"Dane MOJEJ strony:\n{json.dumps(mine, ensure_ascii=False)}"
    )
    text = _call_claude(COMPARE_PROMPT, user_msg, max_tokens=1000)
    return _validate_comparison(_parse_json(text, default={"losing": [], "winning": [], "actions": []}))


N8N_WEBHOOK_URL = os.environ.get("N8N_B2B_WEBHOOK_URL") or os.environ.get("N8N_WEBHOOK_URL", "")
N8N_WEBHOOK_PLATNOSC_URL = os.environ.get("N8N_B2B_PLATNOSC_URL") or os.environ.get("N8N_WEBHOOK_PLATNOSC_URL", "")
...
def analyze():
    ...
    with ThreadPoolExecutor(max_workers=2) as ex:
        fut_comp_ana = ex.submit(analyze_site, competitor_text)
        fut_mine_ana = ex.submit(analyze_site, my_text)
        try:
    ...
    try:
        comparison = compare_sites(competitor_result, my_result)
    except APIUnavailableError:
